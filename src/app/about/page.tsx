  ...
  useEffect(() => {
    ...
    const runScripts = () => {
      ...
      function runPreloader(done: () => void) {
        ...
        const iv = setInterval(() => { p = Math.min(100, p + Math.random() * 22 + 10); bar!.style.width = p + '%'; num!.textContent = Math.floor(p).toString(); if (p >= 100) { clearInterval(iv); setTimeout(finish, 200); } }, 90);
